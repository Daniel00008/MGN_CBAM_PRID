...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        ex_out = self.fc(avg_out).view(b, c, 1, 1)
        return x * ex_out.expand_as(x)
...
def make_model(args):
    if args.model == 'MGN_CBAM':
        res = MGN_CBAM(args)
    elif args.model == 'MGN_SE':
        res = MGN_SE(args)
    return res


class MGN_CBAM(nn.Module):
    def __init__(self, args):
        super(MGN_CBAM, self).__init__()
...
class MGN_SE(nn.Module):
    def __init__(self, args):
        super(MGN_SE, self).__init__()
        num_classes = args.num_classes
        ...
        # SE
        self.inplanes_1 = 64
        self.se_attention = SELayer(self.inplanes_1)

        self.inplanes_2 = 256
        self.se_attention2 = SELayer(self.inplanes_2)

        self.inplanes_3 = 512
        self.se_attention3 = SELayer(self.inplanes_3)

        self.inplanes_4 = 1024
        self.se_attention4 = SELayer(self.inplanes_4)
...
#         print('se_attention:', x.size())

        x = self.backone2(x)
#         print('backone2:', x.size())

        x = self.se_attention2(x)
#         print('se_attention:', x.size())

        x = self.backone3(x)
#         print('backone3:', x.size())

        x = self.se_attention3(x)
#         print('se_attention:', x.size())

        x = self.backone4(x)
#         print('backone4:', x.size())
