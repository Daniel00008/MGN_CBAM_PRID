...
parser.add_argument("--datadir", type=str, default="./data/data_PRCV2020", help='dataset directory')
...
parser.add_argument('--num_classes', type=int, default=1295, help='')  # train:906 test: 389
...
parser.add_argument('--load', type=str, default='MGN_adam_all_data', help='file name to load')
    ...
    with open(filename, 'w') as csv_file:
        ...
        for key_q in data_dict.keys():
            ...
            for key_g, value in data_dict[key_q].items():
                write_list.append(int(key_g)-1) # index start from 0
...
class Model(nn.Module):
    ...
    def load(self, apath, pre_train='', resume=-1, cpu=False):
        ...
        if resume == -1:
            print('load model:',os.path.join(apath, 'model', 'model_best.pt'))
...
class RAP(dataset.Dataset):
    ...
    def __init__(self, args, transform, dtype):
        ...
        if dtype == 'query':
            ...
            self.imgs = [os.path.join(data_path, path.strip()) for path in query_file]
...
class Tester():
    ...
    def test(self):
        ...
        query_file_txt = os.path.join(data_path, query_filename)
        query_file = open(query_file_txt, 'r')
        query_inx_list = [path.rsplit('_', 1)[1].split('.')[0] for path in query_file]
        ...
        for i in res_df.index:
            res_dict_i = pd.DataFrame(res_df.loc[i]).sort_values(by=[i], axis=0, ascending=True).to_dict()
